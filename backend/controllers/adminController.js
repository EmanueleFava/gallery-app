...
const updateUser = async (req, res) => {
    ...
    try {
        ...
        if (!admin || admin.ruolo != "admin"){
            ...
            try {
                ...
                if (user.ruolo == "utente"){
                    ...
                    res.status(200).json({messaggio: `utente aggiornato`, updatedUser});
                } else {
                    const update = await User.update({
                        ruolo: "utente"
                    }, {
        ...
        return res.status(500).json({ error: "Errore durante la verifica dell'amministratore." });
    }
}
    ...
    try {
        ...
        if (!admin || admin.ruolo != "admin"){
            ...
            try {
                ...
                const updatedImage = await Image.findOne({ where: { id: image_id } })	
                return res.status(200).json({messaggio: `immagine aggiornata`, updatedImage});
            } catch (error){
                return res.status(404).json({ error: 'images not found', error});
            }
        }
...
module.exports = {  updateUser, readAllUsersImages, readUserImages, updateImage, deleteImage };